...
public class UIManager_Luar : MonoBehaviour
{
    [Header("UI Text")]
    ...
    IEnumerator Start()
    {
        ...
        if (btnMasukGedung != null)
            btnMasukGedung.onClick.AddListener(OnMasukGedungClicked);
        ...
        // Update shortcut UI pertama kali
        if (shortcutUIManager != null)
            shortcutUIManager.UpdateAllItems();
    }
    ...
    void UpdateWarningDisplay()
    {
        ...
        if (kebersihan <= warningThreshold)
        {
            ...
            if (txtWarning != null)
            {
                if (kebersihan <= criticalThreshold)
                {
                    txtWarning.text = " KRITIS! Kebersihan Sangat Rendah! Tekan [1] atau [Z] untuk KERAHKAN PETUGAS!";
                    txtWarning.color = Color.red;
                }
                else
                {
                    txtWarning.text = " Kebersihan Rendah! Tekan [1] atau [Z] untuk KERAHKAN PETUGAS!";
    ...
    void OnMasukGedungClicked()
    {
        SceneManager.LoadScene("Scene_Dalam");
    }
}